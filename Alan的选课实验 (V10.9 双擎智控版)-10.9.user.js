// ==UserScript==
// @name         Alan的选课实验 (V10.9 双擎智控版)
// @namespace    http://tampermonkey.net/
// @version      10.9
// @description  独立并发探针 | 动态AI唤醒机制 | 双模式控制面板 | 彻底防诈尸
// @author       Alan
...
(function() {
    ...
    const BAIDU_API_KEY = "key";
    const BAIDU_SECRET_KEY = "key";
    ...
    function getBaiduToken() {
        return new Promise((resolve) => {
            if (!BAIDU_API_KEY || !BAIDU_SECRET_KEY) return resolve(false);
    ...
    async function getAutoVcode() {
        if (!BAIDU_API_KEY || !BAIDU_SECRET_KEY) return document.getElementById('inp_vcode').value.trim() || "undefined";
        if (!baiduAccessToken) {
            const success = await getBaiduToken();
            if (!success) return document.getElementById('inp_vcode').value.trim() || "undefined";
        }
        ...
        try {
            ...
            return await new Promise((resolve) => {
                const payload = "image=" + encodeURIComponent(base64) + "&language_type=ENG";
                    ...
                    onload: (res) => {
                        try {
                            ...
                            if (data.words_result && data.words_result.length > 0) {
                                let code = data.words_result[0].words.replace(/[^a-zA-Z0-9]/g, '');

                                if (code.length === 4) {
                                    log(`视觉破译成功：[${code}]`, "success");
                                    resolve(code);
                                } else {
                                    log(`AI 识别位数异常 [${code}]，丢弃重试...`, "error");
                                    resolve("undefined"); // 返回 undefined 触发发包失败，进而让选课函数开启下一轮重试
                                }
                            } else {
                                log("AI 未能看清验证码内容", "error");
                                resolve("undefined");
                            }
    ...
    async function selectCourse(task, courseData, isRetry = false) {
        ...
        try {
            ...
            if (submitJson.result && submitJson.result.includes("验证码校验失败")) {
                if (!window.forceOcr && !window.isCaptchaRequired && BAIDU_API_KEY) {
                    window.isCaptchaRequired = true; // 标记系统已开启防刷
                    log(`验证码`, "error");
                    if (!isRetry) return await selectCourse(task, courseData, true); // 原地重发
                }

                log(` 验证码校验错误，重新`, "error");
                if (!BAIDU_API_KEY) stopMonitor();
                return false;
            }
    ...
    function startMonitor() {
        ...
        if (BAIDU_API_KEY && !baiduAccessToken) getBaiduToken();
    ...
    const initUI = () => {
        ...
        box.innerHTML = `
            <div class="drag-header" id="alan_drag">SCU 选课 V10.9 (双擎智控版)</div>
            ...
            <div class="input-row" style="align-items: center; justify-content: space-between; margin-bottom: 12px; font-size: 12px; color: #666; padding-left: 5px;">
                <label style="display: flex; align-items: center; cursor: pointer;">
                    ...
                    <span style="font-weight: 600;">强制 AI 视觉打码 (关=默认极速尝试)</span>
                </label>
            </div>
